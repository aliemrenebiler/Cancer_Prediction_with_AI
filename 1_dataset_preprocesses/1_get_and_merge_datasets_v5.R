...
if (length(gset_names) != length(mss_colnames)) {
  stop("GEO dataset names and MSS status column names do not match.")
}
...
# For every GEO set
for (i in 1:length(gset_names)) {
  # Set current GEO dataset name and its MSS status column name
  gset_name <- gset_names[[i]]
  mss_colname <- mss_colnames[[i]]
  ...
  # Stop the code if no MSS status value in metadata
  if (!(mss_colname %in% colnames(gset_mdata))) {
    stop('No such "', mss_colname, '" column in "', gset_name, '" metadata.')
  }
